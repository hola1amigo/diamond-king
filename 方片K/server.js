...
function createGameServer({ now = Date.now } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (!["index.html", "player.html", "app.js", "player.js", "lobby.js", "styles.css"].includes(file)) { res.writeHead(404); return res.end("Not found"); }
      const data = await fs.promises.readFile(path.join(PUBLIC_DIR, file));
      const type = file.endsWith(".js") ? "text/javascript" : file.endsWith(".css") ? "text/css" : "text/html";
      res.writeHead(200, { "content-type": `${type}; charset=utf-8`, "cache-control": "no-store", "referrer-policy": "no-referrer" });
      res.end(data);
